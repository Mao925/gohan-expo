...
export default function LikePage() {
  ...
  const {
    ...
    mutationFn: ({ answer, targetUserId }: { answer: 'YES' | 'NO'; targetUserId: string }) =>
    ...
    onSuccess: (data) => {
      ...
      if (data?.matched) {
        setMatchMessage(data.partnerName ? `${data.partnerName}さんとマッチしました！` : 'マッチが成立しました！');
    ...
    onError: (err: any) => {
      ...
      if (apiError?.message) {
        ...
        setError('コミュニティ参加が完了していません。再申請してからご利用ください。');
      } else {
        setError('送信に失敗しました');
      }
    }
  ...
  const {
    mutate: mutateRetryJoin,
    isPending: retryJoinPending
  } = useMutation({
    mutationFn: async (payload?: { code?: string }) => {
      if (!token) throw new Error('ログインしてください');
      const code = payload?.code ?? lastCode;
      if (!code) throw new Error('コミュニティコードを入力してください。');
      return apiFetch('/api/community/join', { method: 'POST', data: { communityCode: code }, token });
    },
  ...
  const canResetLikes = process.env.NODE_ENV !== 'production' && Boolean(DEV_RESET_LIKE_ENDPOINT);
  ...
  useEffect(() => {
    if (statusValue === 'UNAPPLIED' && lastCode && token && !retryJoinPending && !autoJoinAttempted.current) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-semibold text-slate-900">ご飯に行きたい？</h1>
        <p className="mt-2 text-sm text-slate-500">一度選んだらやり直しはできません。直感で選んでみましょう。</p>
      </div>
      ...
      {statusLoading ? (
          ...
          {statusValue === 'UNAPPLIED' ? (
            <>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <Button type="button" variant="secondary" disabled={!lastCode || retryJoinPending} onClick={() => handleRetryJoin()}>
              ...
              {!lastCode ? (
                <div className="space-y-2 rounded-2xl border border-orange-100 bg-orange-50 p-4 text-sm text-slate-600">
                  <p className="font-medium text-slate-800">コードがまだ保存されていません</p>
        ...
        <Card className="space-y-3">
          ...
          <Button type="button" variant="secondary" onClick={handleRefreshCandidate} disabled={isResettingLikes}>
            {isResettingLikes ? 'リセット中...' : '候補を更新'}
          </Button>
        </Card>
      ) : candidate ? (
        <div className="flex flex-col gap-6">
          <Card className="text-center">
            <p className="text-sm uppercase tracking-[0.5em] text-slate-400">M E M B E R</p>
            <h2 className="mt-4 text-3xl font-semibold text-slate-900">{candidate.name}</h2>
        ...
        <Card className="space-y-3">
          <p className="text-slate-500">今は表示できるメンバーがいません。シードデータが投入済みか確認し、必要なら管理者に連絡してください。</p>
          <Button type="button" variant="secondary" onClick={handleRefreshCandidate} disabled={isResettingLikes}>
            {isResettingLikes ? 'リセット中...' : '候補を再取得'}
          </Button>
        </Card>
      )}
    </div>
